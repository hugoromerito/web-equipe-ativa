  ...
  return (
    <div className="medical-layout min-h-screen">
      <div className="container mx-auto px-3 sm:px-6 py-4 sm:py-8 max-w-7xl">
        {/* Header Médico Clean */}
        <div className="medical-section mb-4 sm:mb-6">
          <h1 className="medical-section-title text-xl sm:text-2xl md:text-3xl leading-tight">
            Central de Demandas
          </h1>
          
          <p className="medical-section-subtitle text-xs sm:text-sm md:text-base mt-1 sm:mt-2">
        ...
        {/* Results Header - Design Clean - Mobile Optimized */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <div>
              <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-slate-800 flex items-baseline gap-1.5 sm:gap-2">
                {demands.length}
          ...
          {demands.length > 0 && (
            <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-br from-primary/10 to-primary/5 rounded-lg border border-primary/20 self-start sm:self-auto">
        ...
        {/* Demands Grid/List - Design Hospitalar - Mobile Optimized */}
        {demands.length === 0 ? (
          <div className="text-center py-8 sm:py-12 md:py-20">
            <div className="rounded-xl sm:rounded-2xl bg-white shadow-sm border border-slate-100 p-6 sm:p-10 md:p-16 max-w-xl mx-auto">
              <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 mx-auto mb-4 sm:mb-6 md:mb-8 bg-gradient-to-br from-slate-100 to-slate-50 rounded-2xl sm:rounded-3xl flex items-center justify-center shadow-inner">
                <Search className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 text-slate-300" />
              </div>
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-slate-800 mb-2 sm:mb-3">
                Nenhuma demanda encontrada
              </h3>
              <p className="text-slate-500 mb-6 sm:mb-8 text-sm sm:text-base leading-relaxed px-2">
                Não encontramos demandas com os filtros aplicados.<br className="hidden sm:block" />
                <span className="block sm:inline"> Tente ajustar os critérios de busca ou crie uma nova demanda.</span>
              </p>
              <Link href={`/org/${currentOrg}/unit/${currentUnit}/applicant`}>
                <button className="px-5 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-br from-primary to-primary/90 text-white rounded-lg sm:rounded-xl font-semibold text-sm hover:shadow-lg hover:shadow-primary/25 transition-all duration-200 hover:scale-105">
                  ➕ Criar Nova Demanda
                </button>
              </Link>
            </div>
                          ...
                          <div className="flex items-start gap-3 sm:gap-6">
                            ...
                            <div className="flex-1 min-w-0 space-y-2 sm:space-y-3">
                              ...
                              <div className="flex flex-col gap-2 pt-1 sm:pt-2">
                                ...
                                {demand.responsible && (
                                  <div className="flex items-center gap-2 sm:gap-3 px-2.5 sm:px-3 md:px-4 py-2 bg-gradient-to-r from-blue-50 to-blue-50/50 rounded-lg border border-blue-100">
                                    <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0 shadow-md">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-[11px] sm:text-xs md:text-sm font-bold text-blue-900 truncate">{demand.responsible.name}</p>
                                ...
                                {demand.scheduledDate && (
                                  <div className="flex items-center gap-2 sm:gap-3 px-2.5 sm:px-3 md:px-4 py-2 bg-gradient-to-r from-purple-50 to-purple-50/50 rounded-lg border border-purple-100">
                                    <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-md">
                                      <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-[18px] md:h-[18px] text-white" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <p className="text-[9px] sm:text-[10px] text-purple-600 font-semibold uppercase tracking-wider">Agendamento</p>
                                      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                                        <span className="text-[11px] sm:text-xs md:text-sm font-bold text-purple-900">
                                        ...
                                        {demand.scheduledTime && (
                                          <>
                                            ...
                                            <span className="text-[11px] sm:text-xs md:text-sm font-bold text-purple-900">{formatTime(demand.scheduledTime)}</span>
                                          </>
                                        )}
                                      </div>
                                    </div>
                            ...
                            <div className="flex sm:flex-col items-start sm:items-end gap-1.5 sm:gap-2 flex-shrink-0">
                              {/* Status Badge - Mobile Optimized */}
                              <span className={`inline-flex items-center px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 rounded-md sm:rounded-lg md:rounded-xl text-[10px] sm:text-xs font-bold shadow-sm whitespace-nowrap ${
                                demand.status === 'PENDING' ? 'bg-amber-100 text-amber-800 border border-amber-200' :
                                demand.status === 'CHECK_IN' ? 'bg-cyan-100 text-cyan-800 border border-cyan-200' :
                                demand.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
                                demand.status === 'RESOLVED' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                                demand.status === 'BILLED' ? 'bg-green-100 text-green-900 border border-green-200' :
                                demand.status === 'REJECTED' ? 'bg-red-100 text-red-800 border border-red-200' :
                                'bg-slate-100 text-slate-800 border border-slate-200'
                              }`}>
                                {status.label}
                              </span>

                              {/* Data de Criação - Desktop Only */}
                              <div className="hidden md:block text-[10px] lg:text-[11px] text-slate-400 text-right mt-3 lg:mt-4">
                                <div className="flex items-center justify-end gap-1.5">
                                  <User size={11} />
                                  <span className="font-medium truncate max-w-[100px]">{demand.author}</span>
                                </div>
                                <div className="flex items-center justify-end gap-1.5 mt-1">
                                  <Calendar size={11} />
                                  <span className="whitespace-nowrap">{formatLocalDate(demand.createdAt, { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                                </div>
                              </div>
                            </div>
                          </div>
        ...
        <div className="text-center mt-6 sm:mt-12 md:mt-16 pb-4 sm:pb-6 md:pb-8">
          <Link href={`/org/${currentOrg}/unit/${currentUnit}/applicant`}>
            <button className="group relative inline-flex items-center justify-center gap-2 sm:gap-3 w-full sm:w-auto px-5 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 bg-gradient-to-br from-primary via-primary to-primary/90 text-white rounded-lg sm:rounded-xl md:rounded-2xl font-bold text-sm sm:text-base shadow-lg shadow-primary/30 hover:shadow-2xl hover:shadow-primary/40 hover:scale-105 transition-all duration-300 overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
              <Sparkles className="w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 relative z-10 group-hover:rotate-12 transition-transform flex-shrink-0" />
